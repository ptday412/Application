...
def genarate_ai_diary(images, moods, hashtags):
    ...
    print('>>>>>>>>>>>>>>>>tmp_object_key', tmp_object_key)
    print(type(tmp_object_key))
    # print('>>>>>>>>>>>>>>>>>>>>>tmp_object_key[0]', tmp_object_key[0])
    print('>>>>>>>>>>>>>>>>>>>>>tmp_object_key[0].split('.com/')', tmp_object_key.split('.com/'))
    print('>>>>>>>>>>>>>>>>>>>>>tmp_object_key[0].split('.com/')[1]', tmp_object_key[0].split('.com/')[1])
